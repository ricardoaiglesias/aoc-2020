...
pub fn silver(data: &DepartureTable) {
    let earliest: (&usize, usize) = data
        .bus_ids
        .iter()
        .map(|id| (id, earliest_departure_time(data.start_time, *id)))
...
pub fn gold(data: &[(usize, usize)]) -> usize {
    ...
    loop {
        ...
        let bus_state = data
            .iter()
            .map(|(index, id)| (id, will_depart_at(time + index, *id)));
...
fn setup_gold() -> Vec<(usize, usize)> {
    ...
    let num_vec: Vec<(usize, usize)> = vec[1]
        .split(',')
        .map(|x| x.parse::<usize>())
        .enumerate()
