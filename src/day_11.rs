...
impl<'a> Grid<'a> {
    ...
    fn offset(&self, (row, col): (Row, Col), (dy, dx): (isize, isize)) -> Option<(Row, Col)> {
        if(row == 0 && dy == -1) || (col == 0 && dx == -1) { return None; }

        let (new_row, new_col) = ((row as isize + dy) as usize,
                                  (col as isize + dx) as usize);

        if !self.in_bounds((new_row, new_col)) { return None; }
...
fn rules_silver(grid: &[Vec<Cell>], index: (Row, Col)) -> Cell{
    ...
    if state == Cell::Empty && n_occupied == 0 { return Cell::Occupied; }
    if state == Cell::Occupied && n_occupied >= 4 { return Cell::Empty; }
    state
}
...
impl<'v> Iterator for GridLineIterator<'v> {
    ...
    fn next(&mut self) -> Option<Cell> {
        let n_dirs = DIRECTIONS.len();
        if self.direction_index == n_dirs { return None; }
...
fn get_num_occupied_line(grid: &[Vec<Cell>], index: (Row, Col)) -> usize {
   // Create an iterator.
    let iter = GridLineIterator {
        grid: &Grid{ data: &grid, n_rows: grid.len(), n_cols: grid[0].len() },
        index,
        direction_index: 0
    };

    iter.map(|opt| match opt { Cell::Occupied => 1, _ => 0 }).sum()
}
...
fn rules_gold(grid: &[Vec<Cell>], index: (Row, Col)) -> Cell{
    ...
    if state == Cell::Empty && n_occupied == 0 { return Cell::Occupied; }
    if state == Cell::Occupied && n_occupied >= 5 { return Cell::Empty; }
