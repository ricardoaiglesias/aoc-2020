...
pub enum ShipInstruction {
    Movement{ dir: Direction, amount: isize},
    Rotation{ dir: RotationDir, amount: RotationAmount},
    Forwards{ amount: isize}
}
...
impl FromStr for ShipInstruction {
    ...
    fn from_str(s : &str) -> Result<Self, Self::Err> {
        ...
        match action {
            'N' => Ok(Movement { dir: Direction::North, amount}),
            'S' => Ok(Movement { dir: Direction::South, amount}),
            'E' => Ok(Movement { dir: Direction::East, amount}),
            'W' => Ok(Movement { dir: Direction::West, amount}),
            'L' =>  Ok(Rotation { dir: RotationDir::Left, amount: rotation_amt.unwrap()}),
            'R' => Ok(Rotation { dir: RotationDir::Right,  amount: rotation_amt.unwrap()}),
        ...
        match instruction {
            ...
            Rotation{dir , amount} => {
                ...
                let offset_amount : isize = match amount {
                    Quarter => 1, Half => 2, ThreeQuarters => 3
                };
...
impl NavigationInfo {
    ...
    fn advance(&mut self, instruction: &ShipInstruction) {
        ...
        match instruction {
            ...
            Rotation{dir, amount} => {
                ...
                let index = match dir {
                    RotationDir::Left => match amount {
                        Quarter => 3, Half => 2, ThreeQuarters => 1
                    },
            ...
            Forwards{amount: times} => {
                let (x_offset, y_offset) = (self.waypoint_position.0 * times, self.waypoint_position.1 * times);
    ...
    fn manhattan_distance(&self) -> isize { self.ship_position.0.abs() + self.ship_position.1.abs()  }
}
...
fn parse(s : &str) -> ShipInstruction {
    ...
    match action {
        'N' => Movement { dir: Direction::North, amount},
        'S' => Movement { dir: Direction::South, amount},
        'E' => Movement { dir: Direction::East, amount},
        'W' => Movement { dir: Direction::West, amount},
        'L' =>  Rotation { dir: RotationDir::Left, amount: rotation_amt.unwrap()},
        'R' => Rotation { dir: RotationDir::Right,  amount: rotation_amt.unwrap()},
        'F' => Forwards { amount },
        _ => unreachable!()
    }
}
