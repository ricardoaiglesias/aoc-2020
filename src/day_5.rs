...
fn divide_partition(current: Partition, dir: Direction) -> Partition {
    ...
    match dir {
        Direction::UPPER => (midpoint + 1, high),
        Direction::LOWER => (low, midpoint),
    }
}
...
fn pass_to_seat(boarding_pass: &str) -> Seat {
    ...
    Seat {
        row: curr_bf_partition.0,
        col: curr_lr_partition.0,
    }
}

fn seat_to_id(s: &Seat) -> usize {
    s.row * 8 + s.col
}
...
pub fn day_5_soln() {
    ...
    for id in id_vec {
        if id - 1 != prev_id && prev_id != 0 {
            println!(
                "(Gold) Missing seat: ID {} vs {}, so missing is {}",
                id,
                prev_id,
                id - 1
            );
